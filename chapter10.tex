...
This Incident Response Plan (IRP) implements the Incident-Response Planning Policy (IRPP) described in Chapter~\ref{chap:contingency}. It establishes the framework for managing information-security incidents to minimise damage, restore normal operations swiftly, and ensure compliance with regulatory requirements.

\subsection{Scope and Objectives}
The IRP covers incidents such as data breaches, malware outbreaks, and service disruptions affecting GreenGrid Energy's IT and OT environments, including SCADA, ERP, and customer-facing systems. The primary objectives are to minimise operational, financial, and reputational impact, and to restore services as quickly as possible.

\subsection{Incident Response Team}
The Incident Response Team (IRT) consists of the Incident Commander, SOC Analysts, OT-Security Engineer, Communications Coordinator, and Legal Advisor. Contact details for each role are maintained in the secure document repository. The IRT is activated when an incident meets predefined severity thresholds, as outlined in the IRPP (see Chapter~\ref{chap:contingency}).

\subsection{Incident Lifecycle Procedures}
Incident management follows a structured lifecycle:
\begin{enumerate}
  \item \textbf{Detection and Reporting}: Incidents are detected through monitoring channels or employee reports. All employees are required to report suspected incidents to the SOC or on-call IRT within 15 minutes of discovery.
  \item \textbf{Analysis and Classification}: The IRT triages incidents, assesses impact, and classifies severity. Severity 1 events (e.g., critical infrastructure impact) are escalated to the CEO and regulators within one hour.
  \item \textbf{Containment}: The IRT implements short-term and long-term containment strategies to limit the spread and impact of the incident.
  ...
  \item \textbf{Post-Incident Review}: The IRT conducts a root-cause analysis, documents lessons learned, and updates policies and controls as needed.
\end{enumerate}
...
Key metrics such as Mean Time to Detect (MTTD), Mean Time to Respond (MTTR), and incident volume are tracked and reported to the Risk and Compliance Committee. The IRT conducts tabletop exercises semiannually and live drills annually to test and improve incident response capabilities.
...
The Disaster Recovery Plan (DRP) operationalises the Disaster-Recovery Planning Policy (DRPP) from Chapter~\ref{chap:contingency}. It details the procedures for restoring essential ICT and OT services, including SCADA control, cloud analytics, and ERP, to their agreed Recovery Time Objective (RTO) and Recovery Point Objective (RPO) after a disruption.

\subsection{Scope and Recovery Objectives}
The DRP applies to all production data centres, public-cloud regions, and OT substations supporting GreenGrid Energy's core revenue streams. Critical systems such as SCADA, ERP, CRM, and the billing engine are prioritised, with RTO and RPO targets defined in the Business Impact Analysis (see Chapter~\ref{chap:BIA}).
...
The DRP may be invoked when predefined conditions are met, such as significant service disruption or data loss. Only the COO or CISO is authorised to declare a disaster and activate the DRP, as specified in the DRPP.
...
Quarterly functional failover tests and annual full-scope simulations are conducted, with results documented and corrective actions tracked. The DRP is reviewed and updated after each test or major incident.
...
The Business Continuity Plan (BCP) implements the Business-Continuity Planning Policy (BCPP) from Chapter~\ref{chap:contingency}. It ensures that critical business processes—such as energy distribution, customer billing, supply-chain procurement, and R\&D labs—continue at predefined capacity levels during and after disruptive events.

\subsection{Business Impact Analysis Summary}
The BCP is informed by the Business Impact Analysis (BIA) in Chapter~\ref{chap:BIA}, which identifies and prioritises critical business processes and specifies Maximum Allowable Downtime (MAD) for each business unit.
