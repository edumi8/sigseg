...
\begin{longtable}{|p{2cm}|p{4cm}|p{4.5cm}|p{4cm}|}
    ...
    \hline
    RD-101 & Malware infection compromising test systems & A.5.20, A.5.28 & \ac{MFA} on administrator access (A.5.20) and encrypted communication channels (A.5.28) minimise infection risks. \\
    \hline
...
The Customer Services \& Billing (CSB) department manages customer relationship data, billing processes, and financial transactions, using a CRM platform and a Billing Engine.
Risks identified include customer data breaches, CRM service disruptions, billing fraud, and system unavailability.
The following table maps these risks to Annex A controls and documents the applicability of each control.

\begin{longtable}{|p{2cm}|p{3cm}|p{4cm}|p{2cm}|p{4cm}|}
    \caption{Statement of application for Customer Services \& Billing Department assets}                                                                                                                                                                                               \\
    \hline
    \textbf{Asset ID} & \textbf{Threat}                                   & \textbf{Control(s) (Annex A)}                                       & \textbf{Status} & \textbf{Justification for Inclusion/Exclusion}                                                                      \\
    \hline
    \endhead
    CS-CRM-01         & Customer data breach (credential theft, phishing) & A.5.18 (Access Control Policy), A.5.20 (Authentication Information) & Included        & Strong authentication and strict role-based access control are mandatory to protect customer PII stored in the CRM. \\
    \hline
    CS-CRM-01         & CRM service disruption (SaaS outage)              & A.17.1.2 (Availability of Information Processing Facilities)        & Included        & CRM business continuity plans are required to ensure availability and avoid customer service disruptions.           \\
    \hline
    CS-BILL-02        & Billing data tampering (fraud)                    & A.5.32 (Logging), A.5.25 (Cryptographic Controls)                   & Included        & Financial transaction integrity must be protected through audit logging and encryption of billing records.          \\
    \hline
    CS-BILL-02        & Billing service DoS attack (availability loss)    & A.17.2.1 (Redundancy of Information Processing Facilities)          & Included        & Billing infrastructure must have redundancy to ensure continuous operation during disruption scenarios.             \\
    \hline
\end{longtable}

The CSB Department's key security priorities focus on ensuring the confidentiality of customer data, the integrity of billing transactions, and the resilience of CRM and billing services against availability risks.
